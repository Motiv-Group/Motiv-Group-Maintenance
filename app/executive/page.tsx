...
export default async function ExecutiveEstatePage() {
  ...
  const kpis: Kpi[] = [
    { label: 'Stores', value: total, hint: `${data.totalRegions} regions`, icon: <Building2 size={13} /> },
    { label: 'Attention Stores', value: e.counts.attention, hint: `${pct(e.counts.attention, total)}% of estate`, icon: <AlertTriangle size={13} />, tone: e.counts.attention ? 'warn' : 'good' },
    { label: 'Open Work', value: e.openTickets, hint: 'vs last week', icon: <ClipboardList size={13} />, trend: tr(data.trends.openWork) },
    { label: 'Supplier Breaches', value: e.supplierSlaBreaches, hint: `${supplierBreachSuppliers} supplier${supplierBreachSuppliers === 1 ? '' : 's'}`, icon: <Truck size={13} />, tone: e.supplierSlaBreaches ? 'warn' : 'good', trend: tr(data.trends.supplierBreaches) },
    { label: 'Internal Breaches', value: e.internalSlaBreaches, hint: 'Across functions', icon: <ShieldAlert size={13} />, tone: e.internalSlaBreaches ? 'warn' : 'good' },
    { label: 'Pending Approvals', value: e.decisionsPending, hint: `${fmtK(data.pendingDecisionValue)} backlog`, icon: <FileText size={13} /> },
    { label: 'Cost Exposure', value: fmtK(e.costExposure), hint: 'High value items', icon: <Banknote size={13} />, trend: tr(data.trends.cost) },
    { label: 'Repeat Defects', value: data.repeatDefects.length, hint: `${pct(data.repeatDefects.length, e.openTickets)}% of total work`, icon: <Repeat size={13} />, tone: data.repeatDefects.length ? 'warn' : 'good' },
    { label: 'Region Alerts', value: regionAlerts, hint: 'Requiring attention', icon: <Globe2 size={13} />, tone: regionAlerts ? 'warn' : 'good' },
    { label: 'Decisions Required', value: actionableDecisions.length, hint: 'Executive actions', icon: <Gavel size={13} />, tone: 'gold' },
  ]
