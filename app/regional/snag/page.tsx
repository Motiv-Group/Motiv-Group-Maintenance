...
export default async function RegionalSnagPage() {
  ...
  if (regionIds.length) {
    ...
    if (tks.length) {
      ...
      const disputedIds = new Set((disputeRows ?? []).map(d => d.ticket_id))
      const reasonBy = new Map<string, string>()
      for (const s of signoffRows ?? []) if (s.reject_reason) reasonBy.set(s.ticket_id, s.reject_reason)   // latest wins (asc order)
