...
import { RegionalTicketActions } from '@/components/exec/RegionalTicketActions'
...
export default async function RegionalTicketDetailPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: store }, { data: quotes }, { data: updates }, { data: signoffs }, { data: suppliers }] = await Promise.all([
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto">
      ...
      <Card className="p-5">
        ...
        <RegionalTicketActions
          ticketId={t.id} status={t.status} supplierId={t.supplier_id}
          suppliers={(suppliers ?? []).map((s: any) => ({ id: s.id, name: s.company_name }))}
          quotes={(quotes ?? []).map((q: any) => ({ id: q.id, amount: q.amount, status: q.status }))}
          pendingSignoffId={pendingSignoff?.id ?? null}
        />
      </Card>
