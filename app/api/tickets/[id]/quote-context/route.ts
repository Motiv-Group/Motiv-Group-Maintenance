...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Signed display URLs for the ticket's documents (attached via the store's
  // info-request response or the RM's "add extra work") so the supplier can open
  // them from the Submit-quote sheet.
  const infoDocs = Array.isArray(t.info_doc_urls) ? await signManyUrls(t.info_doc_urls as string[]) : []
  ...
  return NextResponse.json({
    ticket: {
      ...
      storeName, photoUrls, infoDocs, quoteRequestedAt,
    },
    declinedQuote,
    declinedVariation,
    approvedVariations,
