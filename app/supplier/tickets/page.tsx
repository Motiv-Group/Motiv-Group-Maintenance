...
export default async function SupplierTicketsPage() {
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-white flex items-center gap-2"><Ticket className="text-[#C6A35D]" size={22} /> Assigned Tickets</h1>
        <p className="text-sm text-slate-400 mt-0.5">Open work assigned to you — acknowledge, update, upload evidence, submit for sign-off.</p></div>
            ...
            <tbody>
              {d.tickets.map(t => (
                <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                  <td className="py-2.5 px-2 text-slate-300">{t.storeName}</td>
                  <td className="px-2 text-white max-w-[220px] truncate">{t.title}</td>
                  <td className="px-2 text-slate-300">{t.priority}</td>
                  <td className="px-2"><span className={t.slaLabel === 'Breached' ? 'text-red-400' : t.acknowledged ? 'text-[#C6A35D]' : 'text-blue-400'}>{t.acknowledged ? t.slaLabel : 'New'}</span></td>
              ...
              {!d.tickets.length && <tr><td colSpan={7} className="py-6 text-center text-slate-500">No open work assigned.</td></tr>}
            </tbody>
          </table>
        </div>
      </SectionCard>
