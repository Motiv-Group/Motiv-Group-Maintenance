...
export function RegionalSignoffReview({ rows }: { rows: SignoffRow[] }) {
  ...
  if (!rows.length) return <Card className="p-8 text-center"><p className="text-sm text-[var(--text-faint)]">No jobs awaiting sign-off.</p></Card>
    ...
    <div className="space-y-3">
      ...
      {rows.map(r => (
        <Card key={r.signoffId} className="p-4 space-y-3">
          <div><p className="text-sm font-medium text-[var(--text)]">{r.title}</p><p className="text-[11px] text-[var(--text-faint)]">{r.storeName}</p></div>
          <div className="flex flex-wrap gap-3 text-xs">
            ...
            {!r.before.length && !r.after.length && !r.coc && <span className="text-[var(--text-faint)]">No evidence attached</span>}
          </div>
