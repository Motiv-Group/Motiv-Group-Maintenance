...
// Shared multi-step "log a ticket/job" wizard used by the Store Manager
// (/client/tickets/new) and the Individual home-owner (/individual/tickets/new).
// Category → Details → Urgency → Photos → Review. Parameterised by category set,
// copy, redirect target and an optional read-only context row (the SM's store).
...
export interface LogTicketWizardProps {
  categories: string[]
  title: string
  subtitle: string
  backHref: string
  ...
  redirectHref: string
  submitLabel: string
  /** Read-only row shown on Review (e.g. the SM's store). Omit for individuals. */
  contextRow?: { label: string; value: string | null }
  /** Step-2 helper copy — store vs home-owner framing. */
  urgencyHint?: string
  descriptionPlaceholder?: string
}

export function LogTicketWizard({
  categories, title, subtitle, backHref, backLabel, redirectHref, submitLabel,
  contextRow, urgencyHint = 'Pick how urgent it is — this sets the priority.',
  descriptionPlaceholder = 'What’s wrong, where, and since when…',
}: LogTicketWizardProps) {
  ...
  function stepError(i: number): string | null {
    ...
    if (i === 2 && !impact) return 'Choose how urgent it is.'
  ...
  async function submit() {
    ...
    try {
      ...
      router.push(redirectHref); router.refresh()
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div><h1 className="text-2xl font-bold text-[var(--text)] flex items-center gap-2"><PlusCircle className="text-emerald-500" size={22} /> {title}</h1>
        <p className="text-sm text-[var(--text-muted)] mt-0.5">{subtitle}</p></div>
        ...
        <div className="min-h-[220px]">
          {step === 0 && (
            <fieldset>
              ...
              <p className="text-xs text-[var(--text-muted)] mt-0.5 mb-3">Select the category that fits best.</p>
          ...
          {step === 1 && (
            <fieldset>
              ...
              <textarea className={`${input} min-h-[150px]`} value={description} onChange={e => setDescription(e.target.value)} placeholder={descriptionPlaceholder} autoFocus />
            </fieldset>
          )}
          ...
          {step === 2 && (
            <fieldset>
              ...
              <p className="text-xs text-[var(--text-muted)] mt-0.5 mb-3">{urgencyHint}</p>
          ...
          {step === 4 && (
            <div>
              ...
              <dl className="divide-y divide-[var(--border)] rounded-xl border border-[var(--border)]">
                {contextRow && <ReviewRow label={contextRow.label}>{contextRow.value ?? '—'}</ReviewRow>}
