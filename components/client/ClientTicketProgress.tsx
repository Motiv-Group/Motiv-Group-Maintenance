...
const STEPS = [
  { label: 'Open',        dot: 'bg-blue-500',    ring: 'ring-blue-500/30',    text: 'text-blue-400' },
  { label: 'In Progress', dot: 'bg-[#C6A35D]',   ring: 'ring-[#C6A35D]/30',   text: 'text-[#C6A35D]' },
  { label: 'Completed',   dot: 'bg-emerald-500', ring: 'ring-emerald-500/30', text: 'text-emerald-400' },
] as const

export function ClientTicketProgress({ status }: { status: string }) {
  const cv = clientVisibleStatus(status as TicketStatus)
  if (cv === null) {
    return <p className="text-center text-sm text-slate-500">This ticket is closed ({status === 'declined' ? 'declined' : 'cancelled'}).</p>
  }
        ...
        return (
          <div key={s.label} className={i < STEPS.length - 1 ? 'flex items-start flex-1' : 'flex items-start'}>
            <div className="flex flex-col items-center gap-1.5 w-20">
              ...
              <span className={`text-[11px] font-medium ${reached ? s.text : 'text-slate-500'}`}>{s.label}</span>
            </div>
