    ...
    <Card className="p-5">
      ...
      {open && (recent.length ? recent.map(t => (
        <Link key={t.id} href={`/client/tickets/${t.id}`} className="flex items-center justify-between gap-2 py-2 -mx-2 px-2 rounded-lg border-b border-[var(--border)] last:border-0 hover:bg-[var(--hover)] transition">
          <div className="min-w-0"><p className="text-sm text-[var(--text)] truncate">{t.title}</p><p className="text-[11px] text-[var(--text-faint)]">{t.jobRef ? `${t.jobRef} · ` : ''}{t.category ?? 'General'} · {formatDateTime(t.createdAt)}</p></div>
          <div className="flex items-center gap-1.5 shrink-0">
            <PriorityBadge priority={t.priority} />
            <span className={`text-[11px] font-semibold ${STATUS_TONE[t.status]}`}>{STATUS_WORD[t.status]}</span>
          </div>
        </Link>
