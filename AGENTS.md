...
Flow across roles: store manager logs a ticket (web form or WhatsApp voice note) → the regional manager validates and **invites suppliers to quote** (`ticket_suppliers` — competitive, several supplier orgs at once) → invited suppliers submit `quotes` → the RM (or the individual owner on standalone tickets — executive is workflow-read-only, SEC-045) approves one via `/api/tickets/[id]/quote-decision`, awarding the ticket (`tickets.supplier_id`) → the supplier schedules and works → submits a **signoff** (`signoffs` table: COC + before/after photos + invoice; review rounds in `signoff_rounds`) → RM approves close-out or raises a snag. All lifecycle moves go through `/api/tickets/[id]/transition`.
