...
- **Brand/CI reference:** fonts + colours are documented in `docs/COMPANY_IDENTITY.md` (system `font-sans` stack — no web font loaded; navy/gold `brand-*` palette; `#C6A35D` gold accent; surface/text CSS vars). Consult it before adding colours or a typeface.
