...
function decorateStore(s: any, now: Date): StoreRow {
  return {
    ...s,
...
function summarise(project: any, stores: ProjectStoreLike[], now: Date): ProjectSummary {
...
export async function loadProject(
  companyId: string,
  projectId: string,
): Promise<{ project: any; stores: StoreRow[]; summary: ProjectSummary } | null> {
  const admin = createAdminClient()
  ...
  const now = new Date()
  const rows = ((stores ?? []) as any[]).map((s) => decorateStore(s, now))
...
export async function loadProjectStore(
  companyId: string,
  storeId: string,
): Promise<{ store: StoreRow; project: any; files: ProjectFileView[] } | null> {
  const admin = createAdminClient()
  ...
  const [{ data: project }, { data: files }] = await Promise.all([
    admin.from('projects').select('*').eq('id', (store as any).project_id).single(),
...
export async function fileCategoryCounts(admin: Admin, storeId: string): Promise<Record<string, number>> {
  ...
  for (const r of (data ?? []) as any[]) counts[r.file_category] = (counts[r.file_category] ?? 0) + 1
  return counts
}
