...
import type { EmailCopy, EmailKey } from '@/lib/settings'
import { getAppSettings } from '@/lib/settings-server'
import { resolveCopy } from '@/lib/emails/defaults'
...
type Handlers = { [K in EmailKey]: (vars: EmailVars[K], overrides: Overrides) => BuiltEmail }

const HANDLERS: Handlers = {
  role_invite: (v, overrides) => {
    ...
    return renderBrandedEmail(copy, { base: v.base, link: v.link })
  },

  supplier_invite: (v, overrides) => {
  ...
  password_reset: (v, overrides) => {
    const copy = resolveCopy('password_reset', overrides, {})
    return renderBrandedEmail(copy, { base: v.base, link: v.link })
  },

  store_welcome: (v, overrides) => {
    ...
    return renderStoreWelcome(copy, { email: v.email, password: v.password, loginUrl: v.loginUrl })
  },

  supplier_added: (v, overrides) => {
