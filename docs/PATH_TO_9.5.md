...
| B6 | **HIGH 1** — Next.js 15/16 upgrade PR (clears advisory list incl. nonce-CSP XSS); retest CSP, Capacitor WebView, auth cookies | Deps | Code | ✅ 2026-07-06 — **next 14.2.35 → 16.2.10** on branch `chore/b6-next-16` (PR pending, based on clean `main`). Async request-API migration via Next's `next-async-request-api` codemod (38 files: `await params`/`searchParams`) + async `createClient()` (`await cookies()`) cascaded to 50 server callers (browser client stays sync); `headers()` async in layout; dropped invalid `instrumentationHook` key. eslint(0 err)/tsc/vitest(313)/`next build` all green locally. ⏳ **still to retest on deploy**: CSP nonce (middleware), Capacitor WebView, auth-cookie session; re-run `npm audit` after merge to confirm the Next advisory list clears. |
