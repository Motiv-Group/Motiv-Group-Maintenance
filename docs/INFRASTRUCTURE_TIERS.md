...
| 10 | **Project report per-store photo ZIP for very large projects** (`POST /api/projects/[id]/report` with `zip:true` bundles every store's full-res before/after/COC originals + a per-store PDF in one request) | Vercel Hobby serverless **~60s duration + ~1 GB memory** — a project with many stores × many full-res photos can time out or OOM while downloading + zipping in memory | Works for typical projects (a handful of stores, a few photos each); generation is synchronous with `maxDuration = 60`. Planned fix: stream the ZIP / move to a background job (or downsample photos in the bundle) | Vercel **Pro/Fluid** (longer duration + more memory) or a background-job/streaming code change |
| 11 | **Cold-start TTFB on SA production traffic** (Speed Insights RES ~63; TTFB ~3.6s, FCP/LCP ~4.8s — frontend metrics all green, so it's pure server latency) | Vercel Hobby serverless functions spin down; sparse SA traffic (~28 visits/wk) means most real visits pay a cold start (~1–3s). No warm/fluid compute on Hobby | Pinned function region to `fra1` (co-located with the Frankfurt Supabase → local DB queries; also closer to SA than the previous US-East default) and cached the per-SSR branding read — kills the ocean hop + one DB round trip, but not the cold start itself | Vercel **Pro** (Fluid compute / warm functions), or organic traffic growth keeping functions warm |
| 12 | *(add the next tier-blocked item here)* | | | |
